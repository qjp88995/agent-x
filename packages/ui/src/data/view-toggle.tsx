...
function ViewToggle({ value, onChange, className }: ViewToggleProps) {
  return (
    <div className={cn('inline-flex items-center rounded-[var(--radius-sm)] border border-border bg-card', className)}>
      <button
        type="button"
        onClick={() => onChange('table')}
        className={cn(
          'inline-flex items-center justify-center h-7 w-7 transition-colors duration-[var(--duration-fast)]',
